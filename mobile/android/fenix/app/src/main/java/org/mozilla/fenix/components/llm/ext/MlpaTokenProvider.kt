...
/** Raised when there is no signed-in account to source an FxA access token from. */
internal class FxaNotSignedIn : Llm.Exception("No signed-in account available for an FxA access token")
...
sealed interface FxaAccessToken {
    ...
    object NotSignedIn : FxaAccessToken

    /** An account is signed in but no token could be obtained. */
    object Unavailable : FxaAccessToken
}
